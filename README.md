...
2.  다운로드한 `d2_tz.exe` 파일을 실행합니다.
3.  화면에 "토큰 설정 필요"라는 문구가 뜨면, 윈도우 우측 하단 작업 표시줄(시스템 트레이)에 있는 빨간색 아이콘을 **우클릭**합니다.
4.  메뉴 최상단의 **`🔑 API 토큰 설정 (d2tz.info)`** 를 클릭하고, 아까 복사해둔 API Key를 붙여넣기 한 뒤 확인을 누릅니다.
5.  화면 우측 상단에 **다음 공역** 정보가, 우측 하단에는 **우버디아 진행도**가 정상적으로 나타납니다.
